...
def Q_vwn(b, c):
    c = nn.functional.relu(c) + (b**2)/4 + 1e-12
    Q = 4*c - b**2
    x = torch.sqrt(Q + 1e-8) + 1e-12
...
def fx_vwn(b, c, rs):
    x = rs + b*torch.sqrt(rs + 1e-8) + c
    catch_nan(x=x)
    return x


def f_aux(A, b, c, x0, rs):
    log_arg =  nn.functional.relu(rs/fx_vwn(b, c, rs)) + 1e-12
    log = torch.log(log_arg)
    f_vwn = (f1_vwn(b, c) - f2_vwn(b, c, x0)*f3_vwn(b, c, x0))
    arc_arg = Q_vwn(b, c)/(2*torch.sqrt(rs + 1e-8) + b + 1e-8)
    arc = torch.arctan(arc_arg + 1e-8)
    part1 = f_vwn * arc
    log2_arg = (torch.sqrt(rs) - x0)**2/fx_vwn(b, c, rs)
    log2 = torch.log(nn.functional.relu(log2_arg) + 1e-12)
...
def f_vwn(rs, z, c_arr):
    ...
    if c_arr.shape[0] != z.shape[0]:
        c_arr = torch.tile(c_arr, [z.shape[0], 1])
    ...
    zeta = f_zeta(z)
    x = aux1 \
    + torch.nan_to_num(dmc/(drpa + 1e-8))*aux2 \
    * zeta*(1 - z**4)/(fpp_vwn + 1e-8) + dmc*zeta*z**4
...
def f_xalpha_x(rs, z, constant):
    x = constant*lda_x_spin(rs, z) + constant*lda_x_spin(rs, -z)
    catch_nan(x=x)
    return x
